...
class Gen3FileUpdate:
    ...
    def __init__(
        ...
        try:
            ...
            raise Exception(
                "gen3 package failed to import (or one of the dependencies). Try installing with `pip install gen3`"
            ) from e
        ...
        if file_guid:
            try:
                self.latest_index = latest_index = self.gen3index.get_latest_version(
                    file_guid
                )
                self.latest_file = (
                    None  # this should always be None as it is already uploaded
                )
                ...
                self.same_md5sum_latest_index_and_new_file = (
                    self.latest_md5sum == self.new_md5sum
                )
            except:
        ...
        if sheepdog_id or sheepdog_file_submitter_id:
            self.get_sheepdog_file_record(
                program, project, sheepdog_id, sheepdog_file_submitter_id
            )
            if self.latest_sheepdog_record:
                self.latest_sheepdog_guid = sheepdog_guid = self.latest_sheepdog_record[
                    "object_id"
                ]
                self.latest_sheepdog_md5sum = (
                    sheepdog_md5sum
                ) = self.latest_sheepdog_record["md5sum"]
            else:
        ...
        if self.latest_sheepdog_record and self.latest_index:
            self.same_guid_latest_sheepdog_and_latest_index = (
                sheepdog_guid == self.latest_guid
            )
            self.same_md5sum_latest_sheepdog_and_new_file = (
                sheepdog_md5sum == self.latest_md5sum
            )
            self.same_md5sum_latest_index_and_sheepdog = (
                self.latest_md5sum == sheepdog_md5sum
            )
            if (
                self.same_md5sum_latest_index_and_sheepdog
                and self.same_guid_latest_sheepdog_and_latest_index
            ):
                print("The latest sheepdog and indexd have same file:GOOD")
            else:
                print(
                    "Be careful -- sheepdog and indexd have different files. Investigate further before updating"
                )

        if self.latest_index:
            if self.same_md5sum_latest_index_and_new_file:
                print(
                    "The new file is the same as latest indexd file. No need to update (unless updating metadata)."
                )
        else:
            print(
                "No indexd guid specified or record found-- need to upload a new record/file? use the .create method"
            )
    ...
    def upload_to_file_storage(self):
        ...
        self.new_index = self.gen3index.update_record(
            self.new_guid, urls=[self.new_file_url]
        )
        return self

    def get_sheepdog_file_record(
        self, program, project, sheepdog_id=None, sheepdog_file_submitter_id=None
    ):
        """
        ...
        try:
            if sheepdog_id:
                ...
                node_record = [
                    record
                    for record in node_records["data"]
    ...
    # map to sheepdog
    def create_sheepdog_file_record(
        self,
        file_node_submitter_id=None,
        cmc_node_submitter_id=None,
        data_category=None,
        ...
        assert hasattr(
            self, "new_index"
        ), "Need to create a new indexd record with new file. Use "
        ...
        self.gen3sheepdog.submit_record(
            program,
            project,
            json_records=self.new_sheepdog_record["core_metadata_collections"],
        )
    ...
    def update_sheepdog_file_record(self):
        ...
        project = self.commons_project
        assert (
            self.latest_sheepdog_record
        ), "No sheepdog record -- check to make sure you entered the correct submitter_id or sheepdog_id (ie node_id)"
